...
class Database {
  ...
  static Stream<List<Teacher>> teachersStream([String? targetGroupId]) {
    Query<Map<String, dynamic>> initialQuery = _firestore.collection(TEACHERS_COLL);
    if (targetGroupId != null)
      initialQuery = initialQuery.where('group', isEqualTo: targetGroupId);

    return initialQuery.snapshots()
      .map((QuerySnapshot<Map<String, dynamic>> query) =>
        query.docs.map<Teacher>(
          (QueryDocumentSnapshot<Map<String, dynamic>> qs) => Teacher.fromFirestore(qs)
        ).toList()
      );
  }
  ...
  static Future<bool> createTeacher(Teacher teacher) async {
    try {
      ...
      await _firestore.collection(GROUPS_COLL)
        .doc(teacher.groupId)
