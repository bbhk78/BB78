...
class AddTeacherAttendance extends GetWidget<AuthController> {
  ...
  Widget build(BuildContext context) {
    ...
    final int numTeachersRecorded = todayRollcall.values.where(
      (TeacherAttendanceDay day) => day.status != AttendanceStatus.unknown
    ).length;
        ...
        child: SingleChildScrollView(
          child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int groupIndex) {
              ...
              final Teacher currTeacher = groupsCtrl.teachers.firstWhere(
                (Teacher teacher) => teacher.groupId == currGroup.id
              );

              final TeacherAttendanceDay currDay = todayRollcall.entries.firstWhere(
                (MapEntry<Teacher, TeacherAttendanceDay> entry) => entry.key.id == currTeacher.id
              ).value;
        ...
        child: TextButton(
          ...
          onPressed: () async {
            final bool isValid = todayRollcall.values
              .where((TeacherAttendanceDay day) => day.status == AttendanceStatus.unknown)
              .isEmpty;
    ...
    children: <Widget>[
      ...
      Flexible(
        ...
        child: IconButton(
          ...
          onPressed: () async {
            final TeacherAttendanceDay? potentialDay =
            await Get.bottomSheet<TeacherAttendanceDay>(
